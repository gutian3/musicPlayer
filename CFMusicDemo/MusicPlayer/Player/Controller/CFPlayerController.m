...
- (void)toggleDisplay
{
    if (_isDisplyCDView) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            self->_lyricsManager.lyricView.alpha = 1.0;
            self->_cdView.alpha = 0;
        } completion:^(BOOL finished) {
            
            self->_isDisplyCDView = NO;
        }];
    ...
    else
    {
        [UIView animateWithDuration:0.3 animations:^{
            ...
            self->_cdView.alpha = 1.0;
            self->_lyricsManager.lyricView.alpha = 0;
        } completion:^(BOOL finished) {
            
            self->_isDisplyCDView = YES;
        }];
    }
